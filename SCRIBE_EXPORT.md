...
[CONFIRMED] 2026-05-21 empirical: Make IML `if(condition; valueIfTrue; valueIfFalse)` appears to use EAGER argument evaluation — all three arguments are evaluated before the function executes, even when the condition determines that only one branch is needed. If the unused branch references a dead variable (e.g., `31.data.slug` when Module 31 was filtered out), the whole IML errors and the SetVariables module fails. Use `ifempty(first; second)` instead — it short-circuits and is Make-native | tags: make-com, iml, eager-evaluation, gotcha
